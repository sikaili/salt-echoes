...
export default function shapeInSpace(sk) {
  const createDebris = (index, x, y, size, color) => ({
    ...
    display: item => {
      ...
      switch (
        Math.floor(
          sk.noise(
            sk.frameCount / 200 + sk.mouseX / 3 + sk.mouseY / 3 + index
          ) * 3
        )
      ) {
        case 0:
  ...
  sk.handleTouchStart = () => {
    ...
    Array(Math.ceil(Math.random() * 25))
      .fill('')
      .map((item, index) => {
        sk.debris?.push(
          createDebris(
            Math.random() * 400 * index,
            sk.mouseX,
            sk.mouseY,
            (Math.random() * sk.width) / 8,
            true
          )
        );
      });
  ...
  sk.draw = () => {
    sk.currentBackgroundColor +=
      (sk.backgroundColor - sk.currentBackgroundColor) * 0.05;
